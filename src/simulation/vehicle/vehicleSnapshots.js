...
export function serializeCar(car, rank, penaltySeconds = 0, dependencies) {
  ...
  return {
    ...
    dnf: Boolean(car.destroyed || car.outOfRace),
...
export function serializeTrainingCar(car, rank, dependencies) {
  const {
    normalizePitIntent,
    PIT_INTENT_NONE,
    serializeObservationPitStop,
  ...
  return {
    ...
    lapTelemetry: {
      ...
      lastLapTime: car.lapTelemetry?.lastLapTime ?? null,
      bestLapTime: car.lapTelemetry?.bestLapTime ?? null,
    },
    signedOffset: car.trackState?.signedOffset ?? 0,
    crossTrackError: car.trackState?.crossTrackError ?? 0,
    surface: car.trackState?.surface ?? 'track',
