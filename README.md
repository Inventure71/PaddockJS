...
For deterministic single-skill training or visual checkpoint comparison, set `rules.modules.tireDegradation.enabled: false` so tyre energy remains fixed while the car still drives through normal steering, throttle, brake, and surface physics. `rules.modules.stalledDnf` is enabled by default and retires cars that are off legal racing or pit surfaces while below `speedThresholdKph` for `maxStoppedSeconds`; set `enabled: false` to allow stuck off-track cars to remain live.
