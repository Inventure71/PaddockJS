...
Lifecycle callbacks are optional and host-owned. PaddockJS emits `onLoadingChange`, `onReady`, `onError`, `onDriverSelect`, `onRaceEvent`, `onLapChange`, and `onRaceFinish`; callback errors are routed to `onError` when provided and do not stop the simulator loop. Race snapshots now include `raceControl.finished`, `raceControl.winner`, and final `raceControl.classification`, and the race canvas shows a package-owned winner banner when the configured race distance is complete.
