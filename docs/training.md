...
The HTTP Policy Runner server path uses this compact convention by default. `/policy/reset` sends `protocolVersion: 2` plus static metadata once, and `/policy/decide-batch` sends only `driverIds`, `vectors`, `previousActions`, `metrics`, and `events`. Servers should read model inputs from `body["vectors"][driver_id]`; rich observation objects and schemas are no longer repeated on every decision. This is a breaking transport contract for policy-server integrations. There is no rich per-decision fallback in Policy Runner server mode; use object/full observation output only in local JavaScript, headless, debug, or explicit state/export flows.
