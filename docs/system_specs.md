...
simulator.setPitIntent('budget', 2); // committed automatic pit request
...
- Snapshots expose automatic three-sector track metadata through `track.sectors` and per-car lap/sector timing through `car.lapTelemetry`, including completed current-lap sector splits, live active-sector elapsed time, per-sector progress, and sector performance classes for overall-best, personal-best, and slower completed sector times.
...
- `initialCameraMode` accepts `'overview'`, `'leader'`, `'selected'`, `'show-all'`, or `'pit'`; invalid values fall back to `'leader'`. The pit camera frames `track.pitLane`, zooms out when needed to keep the full pit lane inside the active race-view safe area, and its control is hidden/disabled if the active track has no pit lane. Zoom buttons and wheel zoom work in every camera mode, including overview, show-all, and pit, and drag-pan creates a temporary free-camera target until a named mode is selected again.
...
- Race completion is part of the simulation snapshot. Cars receive individual `finished`, `finishTime`, `penaltySeconds`, `adjustedFinishTime`, and `classifiedRank` values as they cross the finish distance. The first finisher sets a provisional `raceControl.winner`; final `raceControl.classification` and `raceControl.finished` are set only after every car finishes. Final classification converts unserved drive-through and stop-go penalties into configured time, sorts by `finishTime + penaltySeconds`, applies position-drop and disqualification consequences, then race control switches to `safety-car`, the final order freezes, and the field keeps circulating under safety-car behavior.
- Race rules support package presets and custom module config. Supported rulesets are `paddock`, `grandPrix2025`, `fia2025`, and `custom`. Advanced modules include pit stops, tire strategy, penalties, weather, reliability, and fuel load. The current simulator normalizes all module config, creates/renders track-owned pit-lane geometry on an explicit start/finish straight, treats pit-lane asphalt, working-lane service areas, and garage boxes as legal drivable surfaces, runs automatic bounded pit-train entry/service/exit through the main fast lane and shared team service areas when pit stops are enabled, uses the team queue point as a rolling gate when the service area is free, queues a second team car behind the active service area and moves it forward through a queue-release route only after the active service area is physically clear, requests pit stops from configurable tire-energy thresholds, applies nonlinear tire-grip degradation down to 1%, applies the pit speed limiter only on the main pit lane/working lane, lets the built-in driver AI ride kerbs while recovering through normal steering/throttle/brake decisions before the whole car crosses track limits, and enforces stewarded penalties for collisions, track limits, and tire requirements.
...
- Expert environment code can create a headless `createPaddockEnvironment()` from the `@inventure71/paddockjs/environment` subpath. It requires explicit `controlledDrivers`, accepts normalized actions `{ steering, throttle, brake }` plus optional `pitIntent`, advances only through `step(actions)`, and returns environment-loop JavaScript results with `observation`, `reward`, `terminated`, `truncated`, `done`, `events`, `state`, and `info`. Controlled drivers do not receive tire-threshold automatic pit calls; they request automatic pit service through `pitIntent` and observe pit-lane/service state in `observation[driverId].object.self`.
