...
- Package controls, timing rows, drawer toggles, lower-third actions, and dismiss buttons maintain practical `44px` touch targets, shared visible focus states, and `touch-action: manipulation`. Touch driving or virtual joystick controls are intentionally out of scope.
