...
- A root may own only one live all-in-one mount. A second mount is rejected until the first controller is destroyed; failed initialization clears and releases the root for retry.
...
- Each composable surface key owns one root. Replacing that surface clears its previous root, and different surface keys cannot share one root. Concurrent `start()` calls await one initialization; failure destroys partial state and leaves the controller retryable.
...
- `physicsMode` accepts `'arcade'` and `'advanced'`. The default is `'arcade'` to preserve existing hosts. `'advanced'` is opt-in and keeps cars controlled only through steering, throttle, brake, and pit intent while enabling four-contact tire forces, load transfer, yaw inertia, aerodynamic load/drag, velocity-heading slip, and advanced telemetry. The old `'simulator'` mode name is removed and falls back to `'arcade'` like any other invalid value. Snapshots expose `physicsMode` plus per-car `lateralG`, `longitudinalG`, `gripUsage`, `slipAngleRadians`, `tractionLimited`, `stabilityState`, and the latest `appliedControls`.
...
- Headless environments also accept an optional `externalRenderer` observer hook. It receives `{ snapshot, observation, meta }` on `reset`, `step`, and `resetDrivers`, so external tooling can stream authoritative frames to visualizers without letting that transport mutate simulation state. The environment reuses the step's authoritative observation/public snapshot for that hook instead of taking a second full snapshot solely for transport.
- Controller loop code should use `createPaddockDriverControllerLoop()` instead of duplicating browser/headless stepping. Its controller context groups observations by controlled driver id, includes `orderedObservations` for direct batch tensor assembly, exposes metrics, events, cached specs, and reset-driver ids, and keeps per-driver resets as controller reset hooks when the runtime supports `resetDrivers()`. `previousActions` is the last action map actually applied before the current `actions`; on the first controlled frame after reset it is empty, and after repeated frames it reflects the prior physics step rather than pre-copying the current decision. Stopping or fully resetting the loop invalidates an in-flight async decision so it cannot apply a stale action after cancellation.
...
## Demo Physics Selection

The product demo's automatic races, composable examples, layout presets and headless API example use the package-default `arcade` mode. Advanced physics is explicit in the separate, lazily launched keyboard/sensor lab at `#expert`, visibly labeled Advanced physics. The advanced lab uses the rebuilt planar tire-force model with human keyboard smoothing; it remains an experimental handling approximation. Only a focused, visible race captures driving keys, and blur/hidden/disposal clears held inputs and the keyboard's pending pit request. Package defaults and public mode selection are unchanged.
