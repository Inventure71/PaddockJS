...
Browser mounts generate a fresh procedural seed when `trackSeed` is omitted; explicit procedural track seeds are cached so repeated mounts with the same seed do not regenerate or rebuild the same model. Procedural generation starts from connected coarse region masks, traces the outer boundary, smooths the boundary into centerline controls, validates shape, clearance, local turn accumulation, and per-sample heading jumps, and falls back to another region-derived control set instead of an oval control ring when attempts fail.
