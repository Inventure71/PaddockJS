...
`src/simulation/race/raceProgress.js` owns local surface refresh and broad race-state commit helpers. Local refresh updates current wheel/track state needed by post-motion rule phases without changing timing/order. The broad commit owns progress, lap telemetry, sector timing, timing-line crossings, gaps, rank fields, attack state, DRS latch coordination, and finish evaluation.
