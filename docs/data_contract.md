...
`physicsMode` is optional and accepts `'arcade'` or `'advanced'`. Invalid values, including the removed old `'simulator'` name, fall back to `'arcade'`, which is the default compatibility mode. `'advanced'` keeps the same public action contract but uses four-contact force and yaw-inertia dynamics and exposes additional telemetry fields on snapshots and observations: `velocityX`, `velocityY`, `lateralG`, `longitudinalG`, `gripUsage`, `slipAngleRadians`, `tractionLimited`, and `stabilityState`. In advanced mode, `slipAngleRadians` is derived from the car heading versus actual velocity direction, and each wheel contributes forces from its own surface and normal load; wheel snapshots retain their existing contact-patch shape. `gripUsage` is the maximum requested combined force divided by adhesion over the four contacts; it can exceed one. `tractionLimited` flags this demand threshold, while `stabilityState` distinguishes front/rear demand imbalance and large body slip. Acceleration telemetry is the velocity change over the caller step, projected into the body axes at the start of that step, in m/s²; G divides it by 9.80665. Tire slip at a contact is internal and must not be substituted for the public body `slipAngleRadians`. Terminal and race-control-held advanced-mode cars publish `velocityX: 0` and `velocityY: 0` whenever scalar `speed` is zero, so training loops do not see stale pre-DNF, pre-grid, pre-red-flag, or pre-queue motion. Arcade snapshots publish `velocityX` and `velocityY` as `null`; consumers that need a heading-aligned arcade velocity should derive it from `heading` and `speed`. The rebuilt advanced model intentionally changes trajectories and grip-response distributions; controllers/checkpoints trained on the previous advanced dynamics need reevaluation or retraining. Action shapes and observation schema remain unchanged.
...
`trackSeed` is optional. If omitted, each mounted browser simulator creates a fresh procedural circuit. Passing a `trackSeed` makes the track deterministic; repeated procedural contracts use a bounded eight-entry least-recently-used cache so multiple mounts can reuse the same immutable generated track definition when the resolved generation options also match, while evicted contracts regenerate deterministically. Callers should treat `createProceduralTrack(seed, options)` results as read-only. Calling `restart({ trackSeed })` rebuilds the simulation on the deterministic circuit for that seed.
...
- `driverModel`: optional opaque host metadata. PaddockJS preserves the value on normalized driver data and internal car state for host/controller bookkeeping, but does not interpret it or include it in model observations.
...
```js
{
  ...
  driver: {
    ...
    driverModel: { id: 'driver-policy-v2' },
  },
  vehicle: {
    id: 'budget-bb01',
    name: 'BB-01 Ledger',
    ...
    driverModel: { id: 'vehicle-policy-v2' },
  },
  team: {
    id: 'ledger-racing',
    name: 'Ledger Racing',
...
Entries match drivers by `driverId`. Entry `driverId` values must be unique. `driverNumber` is optional; if omitted, PaddockJS falls back to stable grid order. When `driverNumber` is provided, values must be unique. Opaque `driverModel` metadata may be supplied on the top-level driver, entry driver blueprint, or entry vehicle blueprint; precedence is top-level driver, then entry driver, then entry vehicle.
...
Normal browser ticker frames and expert-driven frames use the same lifecycle path. `onLapChange` and `onRaceFinish` therefore run in either mode, duplicate delivery of one simulation event is suppressed, and separate same-time events remain distinct when their stable event, penalty, driver, car, or sequence identifiers differ.
...
Supported mutable snapshot metadata is caller-owned: penalty `consequences` entries, `usedCompounds`, and `wheelOffsets`; car team `pitCrew` and `pitCrewStats`; and pit-stop `serviceProfile.pitCrew` are copied. Mutating them does not change live enforcement or service behavior, and later live changes do not alter prior snapshots. Optional fields remain absent or undefined as supplied; no crew defaults are added by serialization. The live track reference described below is an explicit exception.
...
- `destroy()`: removes listeners, destroys PixiJS runtime, clears the host root, and releases that root for a future mount. A live all-in-one root rejects a second mount; failed initialization clears and releases it automatically.
...
- `mountTelemetrySectorBanner(root)`: renders the selected driver's compact sector-progress lower-third only.
...
- `start()`: initializes PixiJS, binds mounted controls, and starts the simulation loop. Concurrent callers await the same initialization. If initialization fails, partial runtime state is destroyed and a later call may retry.

Mount component roots before calling `start()`. Each surface key owns one root: remounting that surface into another root clears the prior root, and different surfaces cannot share one root. If a component is not mounted, the runtime skips that UI surface instead of requiring hidden placeholder DOM. Every public mounted surface renders lightweight static package markup and a surface-specific `data-paddock-loading` overlay immediately. `start()` removes those overlays only after PixiJS, assets, controls, initial DOM readouts, and the first frame have initialized. If `restart(nextOptions)` is called before `start()`, mounted component markup is refreshed from the new resolved options so copy, component visibility, driver data, and theme changes do not leave stale pre-start HTML behind.

This loading overlay is the package's client-side startup state. It begins after the PaddockJS JavaScript bundle has executed and a mount function has run. Hosts that need an instant placeholder before package JavaScript loads should render host-owned fallback HTML/CSS around or inside the mount root, then let `mountF1Simulator()` or the composable `mount*()` calls replace that root content when the package starts.

For a package-owned pre-JS placeholder, import `createPaddockLoadingPlaceholder()` from `@inventure71/paddockjs/placeholder` and the optional tiny stylesheet from `@inventure71/paddockjs/placeholder.css`. That subpath is intentionally separate from the root browser mount and does not import PixiJS, simulator assets, package runtime CSS, or `F1SimulatorApp`.
