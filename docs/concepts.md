...
Browser mounts that omit `trackSeed` create a fresh procedural track for that mount. Passing `trackSeed` makes the generated circuit deterministic; repeated procedural seeds are cached within the page runtime. Generated circuits start from connected coarse regions, trace the region boundary, then smooth that boundary into the sampled centerline. This lets tracks move inward and outward through concave sections instead of degrading into circular or oval fallback layouts. Failed candidates retry with another deterministic region-derived shape.
