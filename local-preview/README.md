...
The templates page lazy-starts heavy simulator demos before their host roots enter the viewport. It uses the official `@inventure71/paddockjs/placeholder.css` start-light placeholder markup in the initial HTML so the simulator area paints even if the full preview JavaScript has not loaded yet. This keeps the showcase from initializing every offscreen PixiJS app at once while still giving users an immediate package-owned loading surface until the next visible simulator starts.
